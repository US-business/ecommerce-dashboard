...
import { Pagination, PaginationContent, PaginationEllipsis, PaginationItem, PaginationLink, PaginationNext, PaginationPrevious } from "@/components/shadcnUI/pagination"
import CustomPagination from "@/components/ui/ReusablePagination"
        ...
        <div className="min-h-screen bg-background pt-12">
            <div className="container mx-auto px-4 flex">
                ...
                <AsideBar title={t("search.filter")} iconLeft={<Filter className="h-4 w-4 " />}>
                    <Button variant="outline" size="sm" onClick={clearFilters} className="text-sm flex items-center group mb-4">
                    ...
                    <Collapsible title={t("search.sortBy")}>
                        <Select value={sortBy} onValueChange={setSortBy}>
                            <SelectTrigger className="w-full"><SelectValue placeholder={t("search.sortBy")} /></SelectTrigger>
                            <SelectContent>
                    ...
                    <Collapsible title={t("products.price")}>
                        <div className="px-2 my-4 flex flex-col gap-2">
                            <Slider value={priceRange} onValueChange={setPriceRange} max={10000} min={0} step={10} className="mb-2" />
                            <div className="flex justify-between text-xs text-muted-foreground">
                                <span>{priceRange[0]} LE</span>
                                <span>{priceRange[1]} LE</span>
                            </div>
                            <div className="flex justify-between">
                                <Input type="number" value={priceRange[0]} onChange={(e) => setPriceRange([Number(e.target.value), priceRange[1]])} className="w-20" />
                                <Input type="number" value={priceRange[1]} onChange={(e) => setPriceRange([priceRange[0], Number(e.target.value)])} className="w-20" />
                            </div>
                        </div>
                    </Collapsible>
                    ...
                    <Collapsible title={t("search.availability")}>
                        <div className="flex flex-col gap-4 mb-4">
                            <div className="flex items-center justify-between w-full">
                                ...
                                <Switch id="onSaleOnly" checked={onSaleOnly} onCheckedChange={setOnSaleOnly} />
                            </div>
                            <div className="flex items-center justify-between w-full">
                                <Label htmlFor="inStockOnly">{t("search.inStock")}</Label>
                                <Switch id="inStockOnly" checked={inStockOnly} onCheckedChange={(val) => { setInStockOnly(val); if (val) setOutOfStockOnly(false) }} />
                            </div>
                            <div className="flex items-center justify-between w-full">
                                <Label htmlFor="outOfStockOnly">{t("search.outOfStock")}</Label>
                                <Switch id="outOfStockOnly" checked={outOfStockOnly} onCheckedChange={(val) => { setOutOfStockOnly(val); if (val) setInStockOnly(false) }} />
                            </div>
                        </div>
                    </Collapsible>
                </AsideBar>
