...
export async function getCoupons(page = 1, limit = 10, search?: string): Promise<CouponsResponse> {
  try {
    ...
    try {
      ...
      let query = db.select().from(coupons).orderBy(desc(coupons.createdAt)).limit(limit).offset(offset)

      if (search) {
        query = query.where(or(like(coupons.code, `%${search}%`), like(coupons.discountType, `%${search}%`)))
      }
...
export async function createCoupon(data: CouponFormData): Promise<CouponResponse> {
  try {
    await requireSuperAdmin()
...
export async function updateCoupon(id: number, data: CouponFormData): Promise<CouponResponse> {
  try {
    await requireSuperAdmin()
