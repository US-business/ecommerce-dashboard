...
export function Collapsible({ title, children, defaultOpen = true, className, dir = "ltr" }: CollapsibleProps) {
    ...
    return (
        <div className={cn("border-b border-gray-200 last:border-b-1", className)}>
            <Button
                variant="ghost"
                onClick={() => setIsOpen(!isOpen)}
                className={`w-full justify-between px-4 py-2 h-auto font-medium text-left ${dir === "rtl" ? "flex-row-reverse" : ""} ${isOpen? "bg-gray-100" : ""} hover:bg-gray-50`}
