...
const CartAccordionMenu = ({ user, cart, dictionary, dir }: CartAccordionMenuProps) => {
   ...
   return (
      <Accordion type="single" collapsible className="w-full">
         <AccordionItem value="cart-menu">
            <AccordionTrigger className={cn("flex items-center ", dir === "rtl" && "flex-row-reverse")}>
               <div className={cn("flex items-center justify-center flex-1 gap-3", dir === "rtl" && "flex-row-reverse")}>
                  <span className="font-medium">{dictionary.cart.title}</span>
                  <ShoppingCart className="h-5 w-5 ml-1" />
                  {totalItems > 0 && (
                     <Badge
                        variant="destructive"
                        className="h-5 w-5 flex items-center justify-center p-0 text-xs text-white"
                     >
                        {totalItems}
                     </Badge>
                  )}
