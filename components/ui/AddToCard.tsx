...
const AddToCard = ({ productData, dir }: ProductControlsProps) => {
   const { user } = useAuth();
      ...
      <>
         <div className="flex items-center gap-2 w-full ">
            ...
            <button type='button'
               ...
               className="flex-1 bg-amber-600/10 text-amber-950 hover:bg-amber-600/20 border border-amber-600/20 
               h-8 text-md rounded-lg font-normal cursor-pointer hover:border-2 hover:border-color-frontground-700 transition duration-200 flex items-center justify-center gap-3 hover:bg-color-frontground-100 bg-color-frontground-600 hover:text-color text-color-background focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
               onClick={handleAddToCart}
               disabled={availableStock <= 0}
            >
               {dir === 'rtl' ?
                  (<>
                     <span className='flex items-center '>أضف إلى السلة</span>
                     {quantityToCart > 1 ? (<span className='flex items-center'> {`( ${quantityToCart} )`}</span>) : null}
                     <span className='flex items-center'>
                        <ShoppingCart className=" w-5 h-5 mx-1" />
                     </span>
                  </>)
                  :
                  (<>
