...
const SliderRounded = ({ items, dir, className }: SliderRoundedProps) => {
   ...
   return (
      <>
         <div className={`hidden sm:flex max-w-6xl ${className}`}>
            <Carousel
               opts={{
                  align: "start",
                  loop: true,
                  ...
                  //    1536: { slidesToScroll: 5 }, // 👈 ده اللي بيحدد يتحرك بكام عنصر
                  // }
            ...
            >
               <CarouselContent className="my-10 gap-2 sm:gap-1 md:gap-2 lg:gap-6 px-9">
                  {items.map((product) => {
                     ...
                     return (
                        <CarouselItem key={product.id} className="pl-2 md:pl-4 basis-1/3 md:basis-1/3 lg:basis-1/5">
                           <Card className="group transition-all duration-300 bg-transparent">
                              <CardContent className="p-0 gap-2 flex flex-col justify-between select-none">
                                 ...
                                 <div className="relative mx-auto w-20 h-20 rounded-full sm:w-20 sm:h-20 lg:w-30 lg:h-30 bg-gradient-to-b from-amber-200 to-gray-100">
                                    <div
                                       ...
                                       style={{
                                          background: product.image ? `url(${product.image})` : 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)'
                                       }}
                                    >
                                       <Image
                                          src={product.image || "/placeholder.svg"}
                                          alt={dir === "rtl" ? product.nameEn : product.nameAr}
                                          fill
                                 ...
                                 <div className="relative text-center flex items-center justify-center">
                                    <Badge className={cn("text-[1rem] font-medium text-center text-amber-900 relative bg-amber-100  ")}>{dir === "rtl" ?  product.nameAr: product.nameEn}</Badge>
                                 </div>
